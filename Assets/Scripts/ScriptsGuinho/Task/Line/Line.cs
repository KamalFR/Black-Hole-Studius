...
public class Line : MonoBehaviour, IDragHandler, IEndDragHandler
{
    ...
    void Update()
    {
        // Calcula a posi��o m�dia entre os dois pontos para centralizar a linha
        Vector2 direction = head.position - foot.position;

        // Define o tamanho da linha baseado na dist�ncia entre os dois pontos
        float distance = direction.magnitude;

        // Posiciona a linha no meio entre os dois pontos
        lineImage.rectTransform.position = (head.position + foot.position) / 2;

        // Ajusta o tamanho da linha para que ela cubra a dist�ncia entre os pontos
        lineImage.rectTransform.sizeDelta = new Vector2(distance, 100f);

        // Rotaciona a linha para que ela se alinhe com a dire��o entre os pontos
